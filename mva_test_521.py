...
if stat.S_ISREG(mode):
    ...
    plt.ylabel('Throughput (requests/second)')
    plt.ylim(ymin=0, ymax=throughput_max*1.1)
    plt.plot(data['Users'].iloc[s:s + 7], data['Throughput'].iloc[s:s + 7], label='1 Service(s)', marker='o')
    plt.plot(data['Users'].iloc[s + 8:s + 15], data['Throughput'].iloc[s + 8:s + 15], label='2 Service(s)', marker='o')
    plt.plot(data['Users'].iloc[s + 16:s + 23], data['Throughput'].iloc[s + 16:s + 23], label='3 Service(s)',
             marker='o')
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 3))
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel('Concurrency(N)')
    plt.ylabel('Average Response Time (ms)')
    plt.plot(data['Users'].iloc[s:s + 7], data['Avg_Latency'].iloc[s:s + 7], label='1 Service(s)', marker='o')
    plt.plot(data['Users'].iloc[s + 8:s + 15], data['Avg_Latency'].iloc[s + 8:s + 15], label='2 Service(s)', marker='o')
    plt.plot(data['Users'].iloc[s + 16:s + 23], data['Avg_Latency'].iloc[s + 16:s + 23], label='3 Service(s)',
             marker='o')
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 3))
    plt.legend()
    plt.tight_layout()
