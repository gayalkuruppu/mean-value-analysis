...
if stat.S_ISREG(mode):
    ...
    for s in range(no_of_systems):
        plt.plot(lm, throughput[s], label='{} service(s) test'.format(s + 1))
    s = 0
    plt.plot(data['Users'].iloc[s:s + 7], data['Throughput'].iloc[s:s + 7], label='1 Service(s) analytical', marker='o')
    plt.plot(data['Users'].iloc[s + 8:s + 15], data['Throughput'].iloc[s + 8:s + 15], label='2 Service(s) analytical', marker='o')
    plt.plot(data['Users'].iloc[s + 16:s + 23], data['Throughput'].iloc[s + 16:s + 23], label='3 Service(s) analytical',
             marker='o')
    ...
    for s in range(no_of_systems):
        plt.plot(lm, 1000*responseTime[s], label='{} service(s) test'.format(s + 1))
    s = 0
    plt.plot(data['Users'].iloc[s:s + 7], data['Avg_Latency'].iloc[s:s + 7], label='1 Service(s) analytical', marker='o')
    plt.plot(data['Users'].iloc[s + 8:s + 15], data['Avg_Latency'].iloc[s + 8:s + 15], label='2 Service(s) analytical', marker='o')
    plt.plot(data['Users'].iloc[s + 16:s + 23], data['Avg_Latency'].iloc[s + 16:s + 23], label='3 Service(s) analytical',
             marker='o')
    plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 3))
    plt.legend()
    plt.tight_layout()
